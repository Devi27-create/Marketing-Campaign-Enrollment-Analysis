...
Clean Layer Responsibilities:
    ...
    - Preserve row-level grain (no aggregations or KPIs)
    - Prepare canonical datasets for master staging and business-ready layers

Behavior:
    - Drops and fully rebuilds clean-layer tables
    - Creates supporting indexes for downstream performance
...
BEGIN
    ...
    end_time := clock_timestamp();
    RAISE NOTICE 'marketing_opportunity_clean loaded in % seconds',
        EXTRACT(EPOCH FROM end_time - start_time);
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'ERROR in learner_db_clean_dataset: %', SQLERRM;
END;
$$;
